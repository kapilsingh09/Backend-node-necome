import {
  ...
  searchAnimeBySlug,
  fetchGenres
} from '../services/kitsu.service.js';
...
export const getAnimeDetails = async (req, res) => {
  ...
  try {
    ...
    const animeData = {
      ...
      description:
        attributes.description || 'No anime description available.',
    ...
    // Fetch genres
    animeData.genres = await fetchGenres(
    ...
    const episodes = episodesData.map((ep, index) => {
      ...
      return {
        ...
        description:
          epAttr.synopsis || 'No episode description available.',
